...
класс принимает 2 параметра baseUrl и options. 

-`baseUrl` - url для запроса
-`options` - настройки запроса, заголовки, тело, метод запроса.
...
#### Конструктор класса
создает `this._events = new Map<EventName, Set<Subscriber>>();` - мапу установок подписок к html элементам 

#### Поля класса:
 \_`events: Map<EventName, Set<Subscriber>>;`


#### Методы класса
- `on` - Метод, позволяющий осуществить подписку на событие. 
...
#### Конструктор класса
- \_`constructor(protected readonly container: HTMLElement)`
...
### Класс Form 
### Для создания форм, добавляет обработчики событий, отображает переданные поля. Также в этом классе мы добавляем обработчики событий на input и button.
...
### Класс Card
### Отображает карточку товара. Класс card также относится к слою представления (View) приложения и является родительским для всех видов карточек. Он отвечает за отображение карточки товара. Задает карточке данные о категории товара, заголовок, изображение, цена и описание. Класс наследуется от родительского класса `Component`.
...
- `constructor(protected container: HTMLFormElement, protected actions)`
...
### Обеспечивает взаимодействие с серверной частью приложения. Необходим для получения данных о товарах и отправляет заказ на сервер. Наследуется от класса Api

#### Методы:
- `getItems` - получение товаров с бекенда.
- `createOrder` - отправка заказа на сервер.
...
## Общий список событий в приложении

Код, описывающий взаимодействие слоя представления и слоя данных между собой и фактически выполняющий роль презентера. Взаимодействие осуществляется за счет событий генерируемых с помощью брокера событий и обработчиков событий, описанных в `index.ts` 

- `card:change` - изменение массива карточек продукта.
- `card:selected` - должен провоцировать открытие модального окна карточки продукта.
- `card:basket` - добавление товара в корзину.
- `preview:change` - изменение карточки продукта в модальном окне.
- `modal:open` - открытие модального окна.
- `modal:close` - закрытие модального окна.
- `basket:open` - открытие интерфейса корзины.
- `basket:close` - закрытие интерфейса корзины.
- `basket:change`- смена кол-ва товара в корзине на главной странице, изменение суммы товаров в корзине.
